...
// ---------------------------------------------------------------------------
// resolveContext  (use on internal / service-key-only routes if member context
// is useful but not required — e.g. future admin tooling)
//
// Permissive: if no Authorization header is present, req.context.memberId is
// null and the route handler decides what to do. Does NOT enforce auth.
// Do NOT use this on public/frontend routes — use requireMemberContext instead.
// ---------------------------------------------------------------------------
...
async function resolveContext(req, res, next) {
  ...
  const authHeader = req.headers['authorization'];
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    req.context = { clientId, authUserId: null, memberId: null, memberRole: null };
    return next();
  }
  ...
  try {
    ...
    if (clientId) {
      const member = await getMemberByAuthUser(clientId, authUserId);
      if (member) {
        memberId = member.id;
        memberRole = member.role;
      }
...
module.exports = { requireMemberContext, resolveContext };
